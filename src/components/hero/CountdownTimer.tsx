...
export const CountdownTimer = ({ nextTerm }: CountdownTimerProps) => {
  ...
  return (
    <motion.div
      ...
      className="bg-[var(--color-surface)]/90 backdrop-blur-sm rounded-2xl p-6 shadow-lg border border-[var(--color-primary)]/10 mb-24"
    >
      ...
      <div className="flex gap-3">
        {timeUnits.map((unit) => (
          <div key={unit.labelEn} className="text-center w-14 md:w-16">
            <div className="text-3xl md:text-4xl font-bold text-[var(--color-primary)] tabular-nums">
              {String(unit.value).padStart(2, '0')}
            </div>
            <div className="text-xs text-[var(--color-text-muted)]">{unit.label}</div>
          </div>
        ))}
      </div>
    </motion.div>
